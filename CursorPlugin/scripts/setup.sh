...
# pairing it with the Vibez iPhone app. Cursor flavor — same ID file
# as the Claude Code and Codex plugins (~/.config/vibez/vibez-id), so
# one ID covers every agent on this Mac.
#
# Invoked by the vibez-cursor npx installer and by notify.sh for
# first-run ID generation.
#
# Args:
...
case "${ACTION}" in
    ...
    test)
        if [ -z "${ID}" ]; then
            printf 'No Vibez ID configured yet. Run `npx vibez-cursor` first.\n'
            exit 0
        fi
        body=$(printf '{"vibezId":"%s","title":"vibez test","body":"If you can read this on your phone, the Cursor plugin is wired up.","agent":"cu","event":"done","shield":"on"}' "${ID}")
