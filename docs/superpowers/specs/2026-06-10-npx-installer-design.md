...
One command — `npx vibez` — installs the Vibez plugin into every supported agent CLI
...
- npm name: **`vibez`** (bare name confirmed free), published from `installer/` in this repo.
- Zero runtime dependencies, single ESM file `cli.mjs`, `bin: { vibez: "./cli.mjs" }`,
  Node >= 18, MIT.

## Behavior of `npx vibez`
...
`npx vibez` becomes the headline install in the root README and both plugin READMEs;
