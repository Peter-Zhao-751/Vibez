...
case "${ACTION}" in
    ...
    test)
        if [ -z "${ID}" ]; then
            printf 'No Vibez ID configured yet. Run /vibez:setup first.\n'
            exit 0
        fi
        body=$(printf '{"vibezId":"%s","title":"vibez test","body":"If you can read this on your phone, the plugin is wired up.","agent":"cc","event":"done","shield":"on"}' "${ID}")
